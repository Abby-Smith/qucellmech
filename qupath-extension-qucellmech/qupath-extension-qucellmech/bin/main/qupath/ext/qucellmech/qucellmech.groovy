...
for (entry in project.getImageList().findAll { selectedImageNames.contains(it.getImageName()) }) {
    ...
    // make sure all nuclei are saved as annotations, not detections
    def nuc_detections = imgHi.getDetectionObjects()
    def nucs = nuc_detections.collect {
        return PathObjects.createAnnotationObject(it.getROI(), it.getPathClass())
    }
    imgHi.removeObjects(nuc_detections, true)
    imgHi.addObjects(nucs)
...
if (runCellpose) {
    for (entry in project.getImageList().findAll { selectedImageNames.contains(it.getImageName()) }) {
        ...
        if (!annotations.isEmpty()) {
            ...
            exportObjectsToGeoJson(annotations, gjPath, "FEATURE_COLLECTION")
